...
public class AionGenesisPoS extends AionPoSBlock {
    /**
     ...
     *
     * <p>These are constants that will be used by {@link AionGenesisPoS} to be overidden by the
    ...
    /** Corresponds to {@link AionGenesisPoS#premine} default premined accounts */
    ...
    public static class Builder {
        ...
        public Builder withParentHash(final byte[] _parentHash) {
            parentHash = _parentHash;
            return this;
        }

        public Builder withCoinbase(final AionAddress _coinbase) {
            coinbase = _coinbase;
            return this;
        }

        public Builder withDifficulty(final byte[] _difficulty) {
            difficulty = _difficulty;
            return this;
        }

        public Builder withNumber(final long _number) {
            if (_number < 0) throw new IllegalArgumentException("number cannot be negative");

            number = _number;
            return this;
        }

        public Builder withTimestamp(final long _timestamp) {
            if (_timestamp < 0) throw new IllegalArgumentException("timestamp cannot be negative");

            timestamp = _timestamp;
            return this;
        }

        public Builder withSeed(final byte[] _seed) {
            seed = _seed;
            return this;
        }

        public Builder withEnergyLimit(final long _energyLimit) {
            if (_energyLimit < 0)
        ...
        public Builder withChainId(final int _chainId) {
            if (_chainId < 0) throw new IllegalArgumentException("chainId cannot be negative");
        ...
        public Builder addPreminedAccount(final AionAddress address, final AccountState state) {
            if (premined == null) this.premined = new HashMap<>();

            if (premined.get(address) != null)
                throw new IllegalArgumentException("duplicate premined address");

            premined.put(address, state);
            return this;
        }
         ...
         */
        public AionGenesisPoS build() throws HeaderStructureException {
            if (parentHash == null) parentHash = GENESIS_PARENT_HASH;

            if (coinbase == null) coinbase = GENESIS_COINBASE;

            if (difficulty == null) difficulty = GENESIS_DIFFICULTY;

            if (number == null) number = GENESIS_NUMBER;

            if (timestamp == null) timestamp = GENESIS_TIMESTAMP;

            if (seed == null) seed = GENESIS_SEED;

            if (energyLimit == null) energyLimit = GENESIS_ENERGY_LIMIT;

            if (premined == null) premined = GENESIS_PREMINE;

            if (networkBalance == null) networkBalance = GENESIS_NETWORK_BALANCE;

            byte[] extraData = generateExtraData(chainId);
            ...
            genesis.setPremine(premined);
            genesis.setNetworkBalance(networkBalance);
