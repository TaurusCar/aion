...
final class TaskImportBlocks implements Runnable {
    ...
    public void run() {
        ...
        while (start.get()) {
            ...
            if (peerState == null) {
                ...
                List<AionPoSBlock> batch = filterBatch(bw.getBlocks(), chain, importedBlockHashes);
     ...
     */
    @VisibleForTesting
    static List<AionPoSBlock> filterBatch(
            List<AionPoSBlock> blocks,
    ...
    private PeerState processBatch(PeerState givenState, List<AionPoSBlock> batch, String displayId) {
        ...
        if (givenState.getMode() != BACKWARD) {
            AionPoSBlock b = batch.get(batch.size() - 1);
        ...
        for (Block b : batch) {
            try {
                importResult = importBlock((AionPoSBlock) b, displayId, givenState);
     ...
     */
    static boolean isAlreadyStored(AionBlockStore store, AionPoSBlock block) {
        return store.getMaxNumber() >= block.getNumber() && store.isBlockExist(block.getHash());
    }

    private ImportResult importBlock(AionPoSBlock b, String displayId, PeerState state) {
        ImportResult importResult;
    ...
    private int importFromStorage(PeerState state, long first, long last) {
        ...
        while (level <= last) {
            ...
            Map<ByteArrayWrapper, List<AionPoSBlock>> levelFromDisk =
            ...
            for (Map.Entry<ByteArrayWrapper, List<AionPoSBlock>> entry : levelFromDisk.entrySet()) {
                // initialize batch counter
                batch = 0;

                List<AionPoSBlock> batchFromDisk = entry.getValue();
                ...
                for (AionPoSBlock b : batchFromDisk) {
                    try {
